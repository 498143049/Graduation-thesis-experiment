...
def _raw_larks(Inimg, wsize=5, interval=1, h=2, alpha=0.13, ):
    """
    ...
    ds, unlark, LARK = _make_lark(inimg_std, wsize, h, interval, alpha)
...
def _make_visual_lark(larks, M, N, wsize_skr):
    ...
    out_larks = [[[] for i in range(0, N - wsize_skr + 2, wsize_skr)] for i in range(0, M - wsize_skr + 2, wsize_skr)]
    ii = 0
    for i in range(0, M - wsize_skr + 1, wsize_skr):
        jj = 0
        for j in range(0, N - wsize_skr + 1, wsize_skr):
            out_larks[ii][jj] = np.array(
                larks.T[j + win_skr][i + win_skr].reshape(wsize_skr, wsize_skr, order='F').copy())
            jj = jj + 1
        ii = ii + 1
